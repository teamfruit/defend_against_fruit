...
import os
import shutil
from file_management import ensure_path_exists, split_ext, rm, get_submodule_info, compute_repo_path_from_module_name
from file_management import MODULE_INFO_DIR
from exec_utils import run_nosetests, install_dev, build_sdist
from build_info_utils import merge_module_info_files, build_info_to_text, get_deploy_functions
...
def _extract_single_module_from_build_info(build_info):
    if len(build_info.modules) != 1:
        raise RuntimeError("One and only one module is expected in the build info of a single module build.")
...
def deploy_module(env_info, verify_cert=True):
    ...
    build_info = merge_module_info_files(build_info_files=(module_info_file,), env_info=env_info)
    ...
    deployed_files = [deploy_artifact(path=path, glob_patterns=[os.path.join('dist', '*')])]
...
def _execute_py_run(script):
    ...
    # To "build" the script, simply copy it into the 'dist' directory and tag it with the version.
...
def _built_script_destination(script, version):
    ...
    build_filename = '{base}-{version}.{ext}'.format(base=base, version=version, ext=ext)

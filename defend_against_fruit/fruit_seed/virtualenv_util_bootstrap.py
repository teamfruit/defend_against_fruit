...
pypi_server = config_file_global_settings.get('pypi_pull_server', None)
virtualenv_util_version = config_file_global_settings.get('virtualenv_util_version', None)
virtualenv_util_path = config_file_global_settings.get('virtualenv_util_path', None)
...
if virtualenv_util_path is not None:
    ...
    if not os.path.isfile(os.path.join(virtualenv_util_path, 'virtualenv_util.py')):
        print('Could not find local {} package, attempting to download...'.format(VIRTUALENV_UTIL_PACKAGE_NAME))
        virtualenv_util_path = None
    else:
        virtualenv_util_path = os.path.normpath(os.path.join(ROOT_PATH, virtualenv_util_path))
        virtualenv_util_local = True

else:
    # Look for something that appears like a virtualenv_util package already in the
    # root directory.  We do not attempt to re-bootstrap (redownload the
    # virtualenv_util package) if one of the correct version already exists.
    for item in os.listdir(ROOT_PATH):
        possible_dir = os.path.join(ROOT_PATH, item, VIRTUALENV_UTIL_PACKAGE_NAME)
...
if virtualenv_util_path is None:
    ...
    print('Downloading {} package version {}...'.format(VIRTUALENV_UTIL_PACKAGE_NAME, virtualenv_util_version))
    ...
    if not virtualenv_util_local:
        print('Using previously downloaded {} package version {}...'.format(VIRTUALENV_UTIL_PACKAGE_NAME, virtualenv_util_version))

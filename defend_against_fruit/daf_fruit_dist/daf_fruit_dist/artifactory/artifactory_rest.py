...
def deploy_file(repo_base_url, repo_push_id, path, filename, attributes=None, username=None, password=None,
                verify_cert=True):
    """
    Deploy the file to the /path/ directory at the given URL. A dictionary (or pre-formatted string) of attributes may
    also be supplied.
    """

    def store_hashes_in_headers(headers):
        md5, sha1 = get_file_digests(filename, digests=(hashlib.md5(), hashlib.sha1()))
    ...
    def generate_uri():
        ...
        if attributes:
            if isinstance(attributes, dict):
                uri += ';' + ';'.join('{}={}'.format(k, v) for k, v in attributes.iteritems())
            elif isinstance(attributes, basestring):
                uri += ';' + attributes
            else:
                raise TypeError(
                    '"attributes" must be either a dictionary or a pre-formatted string of "key1=value1;key2=value2"'
                    ' pairs')
        return uri
    ...
    def upload_file(deploy_uri):
        ...
        with open(filename, 'rb') as f:
            response = requests.put(deploy_uri, data=f, auth=auth, headers=headers, verify=verify_cert)
...
def deploy_globbed_files(repo_base_url, repo_push_id, path, glob_patterns,
                         attributes=None, username=None, password=None, verify_cert=True):
    """
    Like deploy_file, except this function takes a list of globbing patterns. All files (NOT directories) matched by
    these patterns are deployed to the server.
    """
    ...
    # Create a version of deploy_file() with every field filled out except for filename.
    deploy = partial(
        deploy_file,
    ...
    # Set of all files being uploaded. Note that a set is being used here instead of a list so that files matched by
    # more than one globbing pattern are only uploaded once.
    filenames = set()
...
def build_promote(username, password, repo_base_url, build_name, build_number, promotion_request, verify_cert=True):
    ...
    put_req = requests.post(uri, data=json_to_put_on_wire, headers=headers, auth=auth, verify=verify_cert)
...
def publish_build_info(username, password, repo_base_url, build_info, verify_cert=True):
    ...
    put_req = requests.put(uri, data=json_to_put_on_wire, headers=headers, auth=auth, verify=verify_cert)
...
def determine_checksums(username, password, repo_base_url, repo_pull_id, file_path, verify_cert=True):
    ...
    get_response = requests.get(uri, headers=_make_headers(), auth=auth, verify=verify_cert)
    ...
    if 'checksums' in response_json:
        ...
        raise RuntimeError("Artifact found in Artifactory but no checksums were available.")

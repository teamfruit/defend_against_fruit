...
class BuildInfoModuleGenerator(object):
    """
    Incrementally build up a module level meta-data file for later inclusion in an Artifactory build-info message.

    Setuptools/distribute commands are provided with an array of tuples
    accessed at self.distribution.dist_files
    This along with other information in self.distribution provides the information
    necessary to create the artifacts portion of the build-info meta-data
    Artifactory needs. The caveat is only files built by commands within the same
    ...
    To ensure the collection of a full set of Module information the current
    design simply writes a meta-data file into the dist directory.
    Each execution of the custom command enhances the meta-data file to incorporate
    new information.

    There are surely better ways to achieve a similar goal, but most of them involve
    customizing existing distribute commands or introspecting existing binaries in
    the dist directory.

    Customizing the existing distribute commands would effectively be a
    fork of the distribute code base, and therefore difficult to maintain.
    Even a monkey patch of the distribute commands would likely be a bit fragile.

    Similarly introspection turns out to not be sufficiently deterministic.
    The pkg-info in each distribution doesn't make it easy to tell the difference
    between a bdist and an sdist. It is possible to make a good guess, but
    that isn't really ideal.

    Continuous integration/deployment builds are already outside the scope of the distribute
    package. Furthermore, only a build within a continuous integration server should
    typically be able to upload artifacts to a build artifact repository such as Artifactory.
    Therefore, ensuring the continuous integration tooling always
    calls a specialized meta-data collecting command, along with any setup command producing
    ...
    def _create_module_builder(
        ...
        if force_clean or (not os.path.exists(module_file)):
            ...
            if module_id != existing_module.id:
                msg = "module id {} read from {} doesn't match specified value of {}".format(
                    existing_module.id,
                    module_file,
                    module_id
                )
    ...
    def _determine_type_from_command_name(self, command_name):
        ...
        if lc_command_name in self._command_to_type_dict:
            ...
            raise ValueError("unrecognized artifact command: {}".format(command_name))
        ...
        with open(requirement_file, 'r') as req_f:
            for line in req_f:
                ...
                if stripped and not stripped.startswith('#'):
                    requirements.append(pkg_resources.Requirement.parse(stripped))
        return requirements
...
class BuildInfoModuleGeneratorTestHelper(object):
    def __init__(self):
        ...
        self.module_id = Id(group_id=PYTHON_GROUP_ID, artifact_id="pointy-stick", version="1.2.3")
        self.alternate_module_id = Id(group_id=PYTHON_GROUP_ID, artifact_id="very-pointy-stick", version="7.8.9")
        ...
        #Dependency types are unknown so therefore have been intentionally set to None.
        colorama_id = Id(group_id=PYTHON_GROUP_ID, artifact_id="colorama", version="0.2.4")
        ...
        mock_id = Id(group_id=PYTHON_GROUP_ID, artifact_id="mock", version="1.0.1")
        ...
        nose_id = Id(group_id=PYTHON_GROUP_ID, artifact_id="nose", version="1.2.1")
        ...
        unfindable_module_id = Id(group_id=PYTHON_GROUP_ID, artifact_id="unfindablemodule", version="999.999.999")
        ...
        def _add_file_path_and_checksum_to_dictionary(dictionary, dependency):
            dep_file_path = BuildInfoModuleGeneratorTestHelper._compute_synthetic_path(
        ...
        _add_file_path_and_checksum_to_dictionary(self.file_path_to_checksums, self.colorama_dependency)
        _add_file_path_and_checksum_to_dictionary(self.file_path_to_checksums, self.mock_dependency)
        _add_file_path_and_checksum_to_dictionary(self.file_path_to_checksums, self.nose_dependency)

    @staticmethod
    def _compute_synthetic_path(pkg_name, pkg_version):
        path = "{python_group_id}/{pkg_name}/{pkg_name}-{pkg_version}.tar.gz".format(
            python_group_id=PYTHON_GROUP_ID, pkg_name=pkg_name, pkg_version=pkg_version)

        return path

    def determine_file_path(self, pkg_name, pkg_version):
        findable_pairs = (
            (self.colorama_dependency.id.artifact_id, self.colorama_dependency.id.version),
            (self.mock_dependency.id.artifact_id, self.mock_dependency.id.version),
            (self.nose_dependency.id.artifact_id, self.nose_dependency.id.version)
        )
    ...
    def determine_checksums_from_file_path(self, file_path):
        if file_path in self.file_path_to_checksums:
            ...
            raise AssertionError("Unexpected file_path {} within test fixture.".format(file_path))
...
def _create_temp_file_name():
    temp_file = tempfile.NamedTemporaryFile(prefix="module_info_test_", suffix=".json", delete=True)
...
def _execute(module_id, dist_files_tuple_array, freeze_file, module_file, force_dependency_rebuild, force_clean,
...
def test_no_module_file__unfindable_requirements_file__no_force_dependency_rebuild__no_force_clean():
...
def test_rest_exception_no_module_file__full_requirements_file__no_force_dependency_rebuild__no_force_clean():
    module_output_file_name = _create_temp_file_name()
...
def test_no_module_file__full_requirements_file__no_force_dependency_rebuild__no_force_clean():
    ...
    _execute(
        ...
        expected_module_properties=helper.module_properties,
        expected_artifacts=(helper.sdist_artifact, helper.egg_artifact),
        expected_dependencies=(helper.colorama_dependency, helper.mock_dependency, helper.nose_dependency)
    )
    os.unlink(module_output_file_name)


def test_no_module_file__unrecognized_command_in_dist_files_tuple():
...
def test_no_module_file__empty_requirements_file__no_force_dependency_rebuild__no_force_clean():
    """An empty requirements file should result in a defined but empty collection of dependencies.

    Do not confuse an empty collection with a collection of None. A collection of None would imply
    no requirements file was processed.
    """
...
def test_existing_module_file__full_requirements_file__no_force_dependency_rebuild__no_force_clean():
    ...
    _execute(
        ...
        expected_artifacts=(helper.bdist_artifact, helper.sdist_artifact, helper.egg_artifact),
        expected_dependencies=(helper.colorama_dependency,)
    )
    os.unlink(module_output_file_name)


def test_existing_module_file__non_matching_module_id():
...
def test_existing_module_file__full_requirements_file__no_force_dependency_rebuild__force_clean():
...
def test_existing_module_file__full_requirements_file__force_dependency_rebuild__no_force_clean():
    """pre-existing module file with incomplete dependencies but a forced dependency rebuild"""
...
def test_existing_module_file_with_no_artifacts__full_requirements_file__no_force_dependency_rebuild__no_force_clean():
    """pre-existing module file with no artifacts, matching id and different properties"""
...
def test_existing_module_file_with_no_deps__full_requirements_file__no_force_dependency_rebuild__no_force_clean():
    """pre-existing module file with no dependencies, matching id and different properties"""
...
def test_no_module_file__missing_requirements_file__no_force_dependency_rebuild__no_force_clean():
    """An empty requirements file should result in a defined but empty collection of dependencies.

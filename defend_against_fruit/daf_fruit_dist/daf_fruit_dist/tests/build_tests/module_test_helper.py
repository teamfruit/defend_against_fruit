...
def create_module_builder(artifacts=None, dependencies=None, treat_none_as_empty=True):
...
def add_some_artifacts(module_builder):
    ...
    module_builder.add_file_as_artifact(type=PYTHON_SDIST, file=fake_artifact_file)
...
def round_trip_to_and_from_wire_format(domain_object, from_json_data_func, domain_object_assertion_func=None):
    #Perform assertions if any on the domain object before getting started
    ...
    #See if the re-hydrated domain object still passes the assertions (if any)
    if domain_object_assertion_func:
        domain_object_assertion_func(domain_object_2)

    #Take the re-hydrated domain object back down to the wire-level format again
